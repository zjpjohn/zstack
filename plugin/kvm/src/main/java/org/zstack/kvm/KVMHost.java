                    ...
                    if (info.isNewAdded()) {
                        flow(new NoRollbackFlow() {
                            ...
                            public void run(FlowTrigger trigger, Map data) {
                                ...
                                int ssh_port = getSelf().getPort();
                                ShellResult ret = ShellUtils.runAndReturn(String.format("ansible -i %s --private-key %s -m setup %s -e 'ansible_ssh_port=%d'",
                                        AnsibleConstant.INVENTORY_FILE, privKeyFile, self.getManagementIp(), ssh_port), AnsibleConstant.ROOT_DIR);
